...
$(document).ready(function() {
    ...
    $(".photo-slider").owlCarousel({
        ...
        nav:true,
        navText: ["<img src='"+assets_url+"images/left-arrow-big.png'>","<img src='"+assets_url+"images/right-arrow-big.png'>"],
        responsive : {
            0 : {
                dots:true,
                nav:false,
    ...
    $('.prev-winner-sliders').owlCarousel({
        ...
        nav: true,
        dots: false,
        navText: ["<i><img src='"+assets_url+"images/angle-left-arrow.svg'></i>", "<i><img src='"+assets_url+"images/angle-right-arrow.svg'></i>"],
        responsive: {
            0: {
                items: 1,
                nav: false,
    ...
    $('.four-item-carousel').owlCarousel({
        ...
        nav: true,
        dots: false,
        navText: ["<i><img src='"+assets_url+"images/left-arrow.svg'></i>", "<i><img src='"+assets_url+"images/right-arrow.svg'></i>"],
        responsive: {
            0: {
                items: 1,
                nav: true
    ...
    $('.date-carousel').owlCarousel({
        ...
        nav: true,
        dots: false,
        navText: ["<i><img src='"+assets_url+"images/left-white-arrow.png'></i>", "<i><img src='"+assets_url+"images/right-white-arrow.png'></i>"],
        responsive: {
            0: {
                items: 2,
                nav: true,
    ...
    if($(window).width() > 767){
        $(".project-start-col .datepicker.approx-date-datepicker").datepicker({
            ...
            dateFormat: "mm-dd-yy",
            showOn: "button",
            buttonImage: ""+assets_url+"images/white-calander-icon.png",
        ...
        $(".project-start-col .datepicker.approx-date-datepicker").datepicker({
            ...
            dateFormat: "yy-mm-dd",
            showOn: "button",
            buttonImage: ""+assets_url+"images/orange-calander-icon.png",
            buttonImageOnly: true,
            onSelect: function() {
                $(".approx-date-datepicker").addClass("highlight-border");
            }
    ...
    if($("#duration-slider").length > 0){
        ...
        slider1.noUiSlider.on('update', function( values, handle ) {
            ...
            $(".input-with-keypress-2").append("week")
            $(".input-with-keypress-3").append("weeks")
...
/* Yamini Contact Us Inquiry Data Insert Section*/
...
$(document).ready( function(){
    ...
    // Change cost when user selects week
    $('#project-cost-dropdown').change(function(e){
        var projectCost = $('option:selected', this).data('cost');
        $('#projectCost').text('$'+projectCost);
    }).trigger("change");
    ...
    $('#applicationForm').submit(function(e){
        ...
        $('.error').text('');
        var data = new FormData(this);
        // $('#submit-application-form').attr('disabled', 'disabled');

        $.ajax({
            type: 'post',
            url: site_url + 'application/store',
            dataType: 'json',
            data: data,
            processData: false,
            contentType: false,
    ...
    //Copy values of short application form to the application sign up form
    $("button.apply-btn").click(function(e){
        var dateValue = $('#application-form-datepicker').val();
        var durationValue = $('#application-form-duration').val();

        // $('#popupDurationDetail').val(durationValue).prop('selected', 'selected').change();
        $('[name="application[start_date]"]').datepicker('setDate',dateValue);
...
$(document).ready(function(){
 ...
 // For search projects From Home Page Start-->
    $("#searchProject").submit(function(e){
        var data = new FormData(this);
        $.ajax({
            type: 'POST',
            url: site_url+'search',
 ...
 // For Project dates Dropdown in search_result Page Start-->
    $('#startYear').on('click',function(){
        $('#startMonth').prop("disabled", false );
    })
    $('#endYear').on('click',function(){
    ...
    $('#startMonth').on('click',function(){
        ...
        $.ajax({
            type:'post',
            url:site_url+'search/getDaysOfMonth',
            dataType: 'json',
            ...
            success: function(response){
                if(response.status==1){
                    ...
                    $('#startDay').html('');
                    $('#startDay').append('<option value="0">dd</option>');
        ...
        $.ajax({
            ...
            success: function(response){
                if(response.status==1){
                    ...
                    $('#endDay').html('');
                    $('#endDay').append('<option value="0">dd</option>');
                    for(var i= 1;i <= response.Days;i++)
                    {
...
//  <--- End
// For More filters of Projects -->
    $('#more_result').on('click',function(){
        var start = parseInt($('#start_week').text());
        var end = parseInt($('#end_week').text());
        $('#startWeek').val(start);
        $('#endWeek').val(end);
        $('#searchResult').submit();
    })
// For reset of Project Dates -->
    $('#resetDates').on('click',function(){
        $('#startYear').prop('selectedIndex',0);
        $('#startMonth').prop('selectedIndex',0);
        $('#startDay').prop('selectedIndex',0);
        $('#endYear').prop('selectedIndex',0);
        $('#endMonth').prop('selectedIndex',0);
        $('#endDay').prop('selectedIndex',0);
    })
    
// For Sorting of Projects Data Start-->
    $(document).on("change", ".project-view-sorting", function() {
        var sortingMethod = $(this).val();
        if(sortingMethod == 'l2h'){
            sortProjectsViewsAscending();
        }
        else if(sortingMethod == 'h2l'){
            sortProjectsViewsDescending();
        }
    });
    function sortProjectsViewsAscending(){
        var projects = $('.project-data');
        projects.sort(function(a, b){ return $(a).data("views")-$(b).data("views")});
        $(".projects-sort-grid").html(projects);
    }
    function sortProjectsViewsDescending(){
        var projects = $('.project-data');
        projects.sort(function(a, b){ return $(b).data("views") - $(a).data("views")});
        $(".projects-sort-grid").html(projects);
    }
   
    //  * Home Page Most Viewed Trending Project Sorting Start *

    //console.log("Run");
    $(".trending-project-slider .owl-stage").addClass("data-count-trending");

    $('.trending-project-slider .owl-item').each(function(){
        var count = $(this).children("a").attr("data-views");
        $(this).attr("data-count",count);
        $(this).addClass("data-count-cls");
    });

    sortProjectsViewsDescendingHomePage();

    function sortProjectsViewsDescendingHomePage(){
        //console.log("After Run");
        var projects = $('.data-count-cls');
        projects.sort(function(a, b){ return $(b).data("count") - $(a).data("count")});
        $(".data-count-trending").html(projects);
    }

   // * Home Page Most Viewed Trending Project Sorting End *

    //  * Home Page Most Viewed Trending Project Sorting Start *

    //console.log("Run");
    $(".affordable-project-slider .owl-stage").addClass("data-count-affordable");

    $('.affordable-project-slider .owl-item').each(function(){
        var count = $(this).children("a").attr("data-cost");
        $(this).attr("data-count",count);
        $(this).addClass("data-count-afford");
    });

    sortAffordableProjectsViews();

    function sortAffordableProjectsViews(){
        //console.log("After Run");
        var projects = $('.data-count-afford');
        projects.sort(function(a, b){ return $(a).data("count") - $(b).data("count")});
        $(".data-count-affordable").html(projects);
    }
