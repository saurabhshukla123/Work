    ...
    if($(window).width() > 767){
      ...
      $(".project-start-col .datepicker.approx-date-datepicker").datepicker({
        ...
        { 
          ...
          dateFormat: "yy-mm-dd",
          showOn: "button",
          buttonImage: "public/images/orange-calander-icon.png",
          buttonImageOnly: true,
          onSelect: function() {
            $(".approx-date-datepicker").addClass("highlight-border");
          }
...
$("#user_form").submit(function(e){
    ...
    $.ajax({
        ...
        success: function(user){
            console.log(user);
            if(user.status == 0){
                $('#useradd_alert').removeClass('alert-success');
                $('#useradd_alert').addClass('alert-danger');
                $('#useradd_alert').css('display','block');
                $('#useradd_msg').text(user.msg);
            }
            if(user.status == 1){
                $('#useradd_alert').removeClass('alert-danger');
                $('#useradd_alert').addClass('alert-success');
                $('#useradd_alert').css('display','block');
                $('#useradd_msg').text(user.msg);
                setTimeout(function(){
...
$("#user_update_form").submit(function(e){
    ...
    $.ajax({
        type: 'post',
        url: site_url+'user_update_db.php',
        dataType: 'json',
        data: data,
        processData: false,
        contentType: false,
        ...
        error: function(){
            $('#user_update_form').removeAttr('disabled');
            ajaxError();
        }
    });
});

// Delete of user in userList Page Start-->
$('.delete_user').on('click' ,function(){
    var id = $(this).data('id');
    $('#delete_Usermodal').attr('data-id', id);
    console.log(id);
})
$('#delete_Usermodal').click(function (e){
    var user_id = $(this).data("id");
    console.log(user_id);
    jQuery.get(
        site_url + 'user_list.php',
        { del: user_id },
        '',
        ...
        if (response.status == 1) {
            ...
            showUserSuccess(response.msg);
            setTimeout(function () {
                document.location.href = 'user_list.php';
            },1000);
        } else {
            showUserError('Try again');
        }
...
//  <-- End
 function showUserSuccess(msg){
     $('#userlist_alert').removeClass('alert-danger');
     $('#userlist_alert').addClass('alert-success');
     $('#userlist_alert').css('display','block');
     $('#userlist_msg').text(msg);
 }
 function showUserError(msg){
     $('#userlist_alert').removeClass('ralert-success');
     $('#userlist_alert').addClass('alert-dange');
     $('#userlist_alert').css('display','block');
     $('#userlist_msg').text(msg);
 }
function removeUserAlert(){
    setTimeout(function(){
        $('#userlist_alert').css({display:'none'});
    },3000);
}
function removeAddUserAlert(){
    setTimeout(function(){
        $('#useradd_alert').css({display:'none'});
    },2000);
}
/*Kajal JS End*/