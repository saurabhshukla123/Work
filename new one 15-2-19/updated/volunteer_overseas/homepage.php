...
$sql1="SELECT COUNT(p.id) as total_projects,c.name AS country_name,c.image AS country_image FROM projects AS p INNER JOIN countries AS c ON p.country_id=c.id GROUP by c.id";
 ...
 $sql_type="SELECT name,id FROM categories";
 $result = $conn->query($sql);
$result1 = $conn->query($sql1); 
$result2 = $conn->query($affordable_query); 
$activity_result = $conn->query($sql_activity);
$activity_result1 = $conn->query($sql_activity);
$type_result = $conn->query($sql_type);
$type_result1 = $conn->query($sql_type); 
		...
		<div class="row pr-3  pt-2">
			<div class="col-md-12">
				...
				<?php while($row = $result->fetch_assoc()) {
                            ?>
					<div class="item ">
						<div style="background-image:linear-gradient(to bottom, rgba(0, 0, 0, 0)0%, rgba(0, 0, 0, 0.06) 10%, rgba(0, 0, 0, 0.15) 22%,rgba(0, 0, 0, 0.72) 75%, rgba(0, 0, 0, 0.84) 89%, rgba(0, 0, 0, 0.91) 100%),url(<?php echo $row["image"]?>); background-repeat: no-repeat; height:320px; background-size:cover;">
			...
			<div class="col-md-12">
				<div class="owl-two owl-carousel owl-theme pb-5">
					...
					<?php while($row1 = $result1->fetch_assoc()) {
                            ?>
					<div class="item" style="background-image: linear-gradient(to bottom, rgba(0, 0, 0, 0)0%, rgba(0, 0, 0, 0.06) 10%, rgba(0, 0, 0, 0.15) 22%,rgba(0, 0, 0, 0.72) 75%, rgba(0, 0, 0, 0.84) 89%, rgba(0, 0, 0, 0.91) 100%),url(<?php echo $row1["country_image"]?>);height:200px;  background-size:cover;">
						<a href="#">
			...
			<div class="col-md-12">
				<div class="owl-three owl-carousel owl-theme pb-5">                        					
					...
					<div class="item">
						...
						<div style="background-image: linear-gradient(to bottom, rgba(0, 0, 0, 0)0%, rgba(0, 0, 0, 0.06) 10%, rgba(0, 0, 0, 0.15) 22%,rgba(0, 0, 0, 0.72) 75%, rgba(0, 0, 0, 0.84) 89%, rgba(0, 0, 0, 0.91) 100%),url(<?php echo $row2["image"]?>);height:320px;  background-size:cover;">
