    ...
    if ($_SESSION['role'] == "organization") {
		...
		{	$_SESSION["city"]="";	
				...
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
					...
					$sql ="INSERT INTO projects(organization_id,activity_id,category_id,title,image,min_age,max_age,overview_description,accommodation_description,impact_description,project_video_url,city_id,country_id,volunteer_house_address,volunteer_house_description,nearest_airport_address,cost_description,project_startdate_description,is_affordable,status,created_at,updated_at) VALUES('$organization','$activity','$category','$title','imgpath','$min_age','$max_age','$overview_description','$accommodation_description',NULL,'$video','$city','$country',NULL,NULL,NULL,NULL,NULL,'yes','1','$c_date',NULL)";
		   ...
		   else
		   {
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				$c_date=date("Y-m-d")." ".date("h:i:s");
				$time =$date1; 
				$sql ="UPDATE projects  SET  `organization_id`='$organization', `activity_id`='$activity', `category_id`='$category', `title`='$title', `image`='img', `min_age`='$min_age', `max_age`='$max_age', `overview_description`='$overview_description',`accommodation_description`='$accommodation_description',`city_id`='$city', `country_id`='$country',updated_at='$c_date' WHERE id='$projectid' ";
				$result = $conn->query($sql);
